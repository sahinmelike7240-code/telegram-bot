...
REMIND_INTERVAL = 7200  # 2 saatte bir kuralları hatırlatır
DELETE_AFTER = 300      # Kural mesajı 5 dakika sonra silinir
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.type != "private":
        ...
        context.job_queue.run_repeating(remind_rules, interval=REMIND_INTERVAL, first=5, chat_id=chat_id, name=str(chat_id))
        info = await update.message.reply_text(f"👋 @{user.username} Bot aktif! Kural hatırlatıcı 2 saatlik döngüye girdi.")
...
async def list_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    uid = str(user.id)
    if not links:
        m = await update.message.reply_text("📭 Henüz onaylanmış link yok.")
        await asyncio.sleep(5); await m.delete(); await update.message.delete()
        return
    ...
    if not new_links:
        ...
        try:
            ...
            warn = await update.message.reply_text(f"⚠️ @{user.username} Önce botu başlatmalısın (@xlinkkontrol_bot)")
...
async def hepsi_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(chat_id=user.id, text=res, disable_web_page_preview=True)
        info = await update.message.reply_text(f"✅ @{user.username} Tüm liste DM gönderildi.")
        await asyncio.sleep(5); await info.delete()
    except: pass
    try: await update.message.delete()
    except: pass
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.startswith(("/liste", "/hepsi", "/stats", "/start", "/kurallar")): return
